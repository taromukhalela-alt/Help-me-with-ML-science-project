...
from backend.domain import physics_engine
from backend.domain.caps_knowledge import (
    PHYSICS_INTENTS,
...
model_path = os.path.join(ARTIFACT_DIR, "intent_model.pkl")
...
try:
    from backend.ml import training as ml_train
...
def favicon():
    return send_from_directory(FRONTEND_BUILD_DIR, "favicon.svg")
...
from backend.mvc.routes import register_feature_routes
from backend.mvc.services.dashboard_service import DashboardService
from backend.mvc.services.note_service import NoteService

# MVC feature modules are registered here so they receive the fully configured
# Flask application and can use explicit services instead of module globals.
